...
module Expr = struct
  ...
  let%expect_test _ = test expr pp_expr "asdf"; [%expect {| Error: '' |}]

  let%expect_test _ =
    test expr pp_expr "1+2"; [%expect {| Binop (Plus, Const (1), Const (2)) |}]
  ...
  let%expect_test _ = test expr pretty "1+2*3"; [%expect {| 1+2*3 |}]
  let%expect_test _ = test expr pretty "0+0*0"; [%expect {| 0+0*0 |}]

  let%expect_test _ =
    let input =
      String.init 900001 (fun n -> if n mod 2 <> 0 then '*' else '1') in
    (* print_endline input;  *)
